                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->zVPpayAmount->setAttribute(Qt::WA_MacShowFocusRect, 0);
    

    // "Spending 999999 zVP ought to be enough for anybody." - Bill Gates, 2017
    ui->zVPpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzVPSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zVPpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzVP_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zVP is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zVP...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zVP in ") +
...
void PrivacyDialog::on_pushButtonSpendzVP_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zVP is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zVP
        sendzVP();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zVP
    sendzVP();
}

void PrivacyDialog::on_pushButtonZVpControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ZVpControlDialog* zVpControl = new ZVpControlDialog(this);
    zVpControl->setModel(walletModel);
    zVpControl->exec();
}

void PrivacyDialog::setZVpControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzVPSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzVP()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Vpub Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zVPpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zVP is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " VP </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zVP</b>";
    ...
    // use mints from zVP selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZVpControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZVpControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zVP require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zVP"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZVP_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zVP require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zVP"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zVP was spent successfully update the addressbook with the label
    ...
    ZVpControlDialog::setSelectedMints.clear();
    ui->labelzVPSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zVP Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Vp, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zVP Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zVP </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zVP "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zVP "));
    ui->labelzVPAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzVPSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzVP->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zVP
        ui->pushButtonMintzVP->setEnabled(false);
        ui->pushButtonMintzVP->setToolTip(tr("zVP is currently disabled due to maintenance."));

        // Spend zVP
        ui->pushButtonSpendzVP->setEnabled(false);
        ui->pushButtonSpendzVP->setToolTip(tr("zVP is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zVP
        ui->pushButtonMintzVP->setEnabled(true);
        ui->pushButtonMintzVP->setToolTip(tr("PrivacyDialog", "Enter an amount of VP to convert to zVP", 0));

        // Spend zVP
        ui->pushButtonSpendzVP->setEnabled(true);
        ui->pushButtonSpendzVP->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}