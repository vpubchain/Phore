...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::VP, nValue);
...
std::string BlockToString(CBlockIndex* pBlock)
{
    ...
    if (pBlock->nHeight == 0)
        ...
        Generated = GetBlockValue(pBlock->nHeight - 1, pBlock->nMnCount);    //modify by lkz
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (vpub.conf).");
            QMessageBox::warning(this, "Vpub Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
