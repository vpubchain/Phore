...
CzVPWallet::CzVPWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZVPSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZVPSeed_deprecated()) {
                    LogPrintf("%s: Updated zVP seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzVPWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZVPCount(nCountLastUsed);
    else if (!walletdb.ReadZVPCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzVPWallet::Lock()
{
    seedMaster = 0;
}

void CzVPWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzVPWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZVP(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzVPWallet::LoadMintPoolFromDB()
{
...
void CzVPWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzVPWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzVPWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzVPWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZVP(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzVPWallet::SeedToZVP(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzVPWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzVPWallet::GenerateDeterministicZVP(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzVPWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzVPWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
