...
VERSION=
commit=false
url=https://github.com/vpubchain/vpub
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the vpub, gitian-builder, gitian.sigs, and vpub-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/vpubchain/vpub
...
then
    ...
    git clone https://github.com/phoreproject/vpub-detached-sigs.git
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit vpub=${COMMIT} --url vpub=${url} ../vpub/contrib/gitian-descriptors/gitian-linux.yml
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit vpub=${COMMIT} --url vpub=${url} ../vpub/contrib/gitian-descriptors/gitian-win.yml
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit vpub=${COMMIT} --url vpub=${url} ../vpub/contrib/gitian-descriptors/gitian-osx.yml
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit vpub=${COMMIT} --url vpub=${url} ../vpub/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../vpub/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/vpub-*.tar.gz build/out/src/vpub-*.tar.gz ../vpub-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../vpub/contrib/gitian-descriptors/gitian-win-signer.yml
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../vpub/contrib/gitian-descriptors/gitian-osx-signer.yml
